...
namespace EndlessGuard.Unit.Editor
{
    ...
    public sealed class EnemyCatalogEditor : UnityEditor.Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            using (new EditorGUI.DisabledScope(true))
            {
                ...
                EditorGUILayout.TextField(new GUIContent("다음 발급 예정 ID", "현재 발급 이력을 기준으로 다음에 생성될 몬스터 ID입니다."), $"ENEMY_{lastIssuedNumber.intValue + 1:D4}");
            }

            EditorGUILayout.Space(5f);
            EditorGUILayout.HelpBox("Assets/Unit/Data/Enemies 폴더만 검색합니다. 기존 ID는 변경하지 않고, ID가 비어 있는 데이터에만 새 ID를 발급합니다. Catalog 목록은 검색 결과로 다시 구성됩니다.", MessageType.Info);

            using (new EditorGUI.DisabledScope(true))
            {
                EditorGUILayout.PropertyField(enemies, new GUIContent("등록된 몬스터 데이터"), true);
            }

            EditorGUILayout.Space(8f);

            if (GUILayout.Button("몬스터 데이터 검색·ID 발급·등록"))
            {
