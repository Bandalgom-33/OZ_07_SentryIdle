...
// 가챠 시스템 전용 UGUI 패널 바인딩 및 6단계 등급/NEW 태그 실시간 획득 로그 뷰어 UI 컨트롤러
public class GachaUI : MonoBehaviour
{
    ...
    // 가챠 완료 이벤트 수신 및 획득 캐릭터 텍스트 로그(6단계 등급 및 NEW 표기) 포맷팅 연산
    // 이유: 1성~6성 등급별 색상 구별 및 최초 획득 유닛의 경우 [NEW!] 태그를 붙여 시각적 피드백을 전달함
    ...
    // 팀원의 UnitGrade 6단계(OneStar~SixStar) 등급별 UI 표시 색상 HEX 코드 변환
    private string GetGradeColorHex(UnitGrade grade)
    {
        switch (grade)
        {
            case UnitGrade.SixStar:   return "#FFD700"; // Gold (6성)
            case UnitGrade.FiveStar:  return "#FF4500"; // Red/Orange (5성)
            case UnitGrade.FourStar:  return "#A335EE"; // Purple (4성)
            case UnitGrade.ThreeStar: return "#0070DD"; // Blue (3성)
            case UnitGrade.TwoStar:   return "#1EFF00"; // Green (2성)
            case UnitGrade.OneStar:   return "#FFFFFF"; // White (1성)
